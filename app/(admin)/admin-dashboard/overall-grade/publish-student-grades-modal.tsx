...
import { useApproveGradesByClassPeriod } from "@/hooks/use-students";
...
export default function PublishStudentGradeModal() {
  ...
  const {
    mutate: approveGrades,
    isPending,
    error,
  } = useApproveGradesByClassPeriod();
    ...
    approveGrades(
      { classId: className, periodId },
      {
        onSuccess: (data) => {
          setIsOpen(false);
          toast.success(
            `${data.approved} student grade${data.approved === 1 ? "" : "s"} published successfully!`,
            {
              position: "top-right",
              style: {
                "--normal-bg":
    ...
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogTrigger asChild>
        <RippleButton
          ...
          className="h-12 cursor-pointer  rounded-md bg-emerald-700 px-4 hover:bg-emerald-600"
        >
          <FileUp className="text-white size-6" />
          <span className="">Publish Student Grade</span>
        </RippleButton>
      </AlertDialogTrigger>
      <AlertDialogContent className="data-[state=open]:zoom-in-0! data-[state=open]:duration-300 sm:max-w-115.75!">
        <AlertDialogHeader className="items-center">
          <div className="bg-emerald-700/20 mx-auto mb-2 flex size-14 items-center justify-center rounded-full">
            <FileUp className="text-emerald-600 size-6.6" />
          </div>
          <AlertDialogTitle
            className={`${fredoka.className} w-full text-center`}
          >
            Are you sure you want to publish these student grades?
          </AlertDialogTitle>
          <AlertDialogDescription className="text-center w-full">
            Once published, the grades for{" "}
            ...
            will be available for parents to view. Please make sure all grades
        ...
        <AlertDialogFooter>
          ...
          <Button
            ...
            className="bg-emerald-700 font-medium text-[13px] hover:bg-emerald-600  focus-visible:ring-emerald-700 h-10 px-5 cursor-pointer rounded-sm text-white duration-300"
          >
            {isPending ? (
              <Spinner className="size-6" />
            ) : (
              <span className="text-[13px] font-medium"> Publish Grade</span>
            )}
          </Button>
        </AlertDialogFooter>
        {error && (
